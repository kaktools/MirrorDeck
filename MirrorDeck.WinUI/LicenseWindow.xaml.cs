...
public sealed partial class LicenseWindow : Window
{
    ...
    private void ConfigureWindow()
    {
        ...
        try
        {
            AppWindow.Resize(new SizeInt32(800, 520));
    ...
    private void LoadLicenseText()
    {
        ...
        try
        {
            paths.Add(Path.Combine(Package.Current.InstalledLocation.Path, "LICENSE.md"));
        }
        catch
        {
            // Packaged path fallback is optional.
        }
        ...
        foreach (var path in paths)
        {
            try
            {
                if (File.Exists(path))
                {
                    LicenseText.Text = File.ReadAllText(path);
                    return;
                }
            }
            catch
        ...
        LicenseText.Text = "LICENSE.md konnte nicht geladen werden.";
    }
